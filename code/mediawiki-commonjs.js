...
async function textGeneration(userInput) {
  ...
  subtopics_list[0] = subtopics_list[0].substring(4)

  // compile the wiki article based on the subtopics
  let wikitext = await openai.createCompletion({
    model: "text-davinci-002",
  ...
  for (let i in subtopics_list) {
    let output = await openai.createCompletion({
        model: "text-davinci-002",
        prompt: `Write a wikipedia article about ${i[1, i.length -1]} in relation to ${userInput}`,
        max_tokens: 1000,
        temperature: 0.1,
        presence_penalty: 1,
    });
    wikitext += `\n\n${i[i, i.length -1]}${output.choices[0].text}`
  }
